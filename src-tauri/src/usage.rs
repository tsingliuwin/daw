...
5. **查询失败时分析原因并给用户选项。** 不要只报错就结束，要根据错误类型给出分析和建议：
   - 存储层级不支持（storage tier / lower meta / odps）→ 该表使用了 MaxCompute 非标准存储，Hologres 不支持访问，**无法通过修改查询解决**。如实告知用户此表不可用，建议换其他表。
   - 权限不足（permission denied）→ 检查数据源连接的用户权限，或换表。
   - 超时 → 缩小查询范围或减少数据量。
   - 其他错误 → 把错误信息如实告诉用户，询问是否换一张表或调整查询条件。
