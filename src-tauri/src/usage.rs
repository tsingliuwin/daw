...
5. **查询失败时分析原因并给用户选项。** 不要只报错就结束，要根据错误类型给出分析和建议：
   - 分区表错误（partition / odps / storage tier）→ 该表可能是分区表，建议用户指定分区范围（如 `WHERE ds='20260801'`），或查最近的分区。
   - 类型不支持（unsupported type）→ 该表有不支持的列类型，建议换一张表或联系管理员。
   - 超时 → 缩小查询范围或减少数据量。
   - 未知错误 → 把错误信息如实告诉用户，询问是否换一张表或调整查询条件。
