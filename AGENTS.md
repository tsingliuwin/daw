...
- **用户画像（users 类）**：目录/渲染/工具早就有，但 preamble 没引导就永远零沉淀。经验：**新机制要生效，必须有一条把「触发时机」写进提示词的路径**。触发词=「用户表达了任何偏好/习惯/纠错」（"我要的是…""这里不对"），立即写 users/default（六板块：角色与关注点/数据 Agent 观点/沟通与交付偏好/常用口径与基准/已拍板决策/纠错记录，纠错原话逐字保留）；合并更新单一权威版本。「数据 Agent 观点」板块（2026-08-31 增设）是从 135 会话 442 条发言全量整理的做数哲学（五主题：唯一真相/语义映射/交付纪律/业务地基/协作方式，原话锚定带日期）——纠错记录记事件、观点板块记原则，两者互补。定期（或用户要求时）把散落的纠错事件向上提炼进观点板块。
...
真实使用 → `tools/review_transcript.py` 复盘 → 修复（配测试）→ 再用验证。首轮复盘（郑州 8 月归因一单）修出的问题，全部有复盘实证。复盘沉淀分两层：**产品方法论**（去业务化的普适原则）进 `docs/DESIGN-PRINCIPLES.md`（开源项目资产，修订须可追溯到复盘）；**具体业务口径与个人偏好**进用户本地知识库（~/.daw okf，不进仓库）：
...
- **write_okf_knowledge 静默吞正文（P0，2026-08-31 第二轮复盘实锤）**：模型 content 首行常回显与 heading 参数同名的标题行，`upsert_block` 追加自己的标题后构成同名重复，`deduplicate` 按「同名保留首次」把第二次标题**连同其后正文一起吃掉**，工具却报「已写入」——当天两处权威口径（苏州团队独立呈现、8月团队维度重收口）落成空标题，用户纠正过的错误下次会话必然重犯。对账手法：jsonl 工具段的 `detail`（写入回执）存了实际收到的 content，与磁盘对账即可判定「模型传空」还是「写入吞掉」。根修=upsert_block 剥离回显同名标题 + store::write 空 content 拒写（fail-loud，指路 update_okf_metadata）；**「工具报成功」不等于「知识落了盘」，复盘必须以磁盘文件为准对账**。112 个历史空标题残桩（正文在模型自带的不同名标题下）无损失、择机清理，复盘脚本已加「知识库体检」自动检测（真损失/残桩分类）。
- **复盘词表不能比模型迟钝**：同轮里模型成功纠正了错误（搜知识、改口径、写纠错记录），复盘脚本的纠正候选词表却漏报「已经说过多次了，没有苏州系这种叫法」——补「已经说过/说过多次/这种叫法」。用户纠正 = 最高价值信号，检测器漏报等于放走沉淀机会。
- **知识库要有代码级变更追溯（2026-08-31 第三轮复盘落地）**：~/.daw/okf 本来就是 git 仓库、每次写入自动 commit，但提交信息只有文件名、diff 没人看——「吞正文」bug 当天 11:09 那笔提交（+4 −2，新增只有一行光杆标题）就是铁证却无人发现，恢复正文只能拿 jsonl 回执人工对账。补齐三件事：① 提交信息带板块名与 +N −M 行（`okf/store.rs` 的 body 级多重集近似 diff，排除 frontmatter 的 updated_at 噪音）；② 写入回执带同样统计，净删 ≥5 行且零新增当场警告模型核对（第二道闸，第一道是剥离回显标题）；③ 复盘脚本新增「git 审计」（最近 6 次知识变更 + 未提交变更检测——手工改知识文件后必须补提交，否则历史断档）。
- **写入路径的匹配必须与读路径的模糊兜底同宽（2026-08-31 第五轮复盘）**：读标题 8/27 就有模糊兜底（防模型漏「（…）」尾巴），写入的 `upsert_block` 却是精确匹配——模型标题手抖一个空格（「2026 年」vs「2026年」）就绕过原位更新，复制出仅空格差异的近似重复板块，且旧份还是被新份显式「取代」的过时口径（修正优先于并存违规）。修复：标题匹配归一化空白（`norm_heading`，读路径对齐写路径），配测试；体检加「仅空格差异重复板块」检测，上线即抓到第二处存量（组织沿革），均已合并留权威版。**同一容错语义在一个产品里只能有一套宽严标准。**
- **复盘工具自身也有盲区：工作区 okf 在 ~/.daw 之外（2026-08-31 第六轮复盘）**：okf 的 selections/tables 属工作区 scope，落盘在 `<工作区路径>/okf/`——工作区可以是磁盘任意位置（如 E:\demo\task），体检和 git 审计只扫 ~/.daw 时整片知识（本轮的「按问题找表总地图」、密训营档案）全部不可见，一度误判为「文件凭空消失」（实为 payload 里的 file 字段指明了真实落盘位置——对账先看 payload）。修复：`okf_roots()` 从 daw.db 的 workspaces 表枚举全部工作区；顺带修复 relpath 跨盘符崩溃。盲区补上后立刻又曝出两处旧时代吞正文受害者（估算参数区间 1417 字、郑州层级 557 字），已从历史回执逐字恢复。
- **简单问题被擅自升级成看板 + 答案不标来源表（第六轮，用户暴怒单）**：用户问「密训营买了多少单」，模型自说自话「本次任务是考研秋季趋势看板」——还编造了两个不存在的知识条目名（monthly_trend_dashboard/promo_calendar_nodes）去加载；后续多轮用户反复追问「用的哪张表」。修复=preamble 新增两条纪律：①「交付纪律」——所问即所答（问数量第一句给数字，不擅自升级成趋势/看板；叫法对不上先说明采用的口径）；②关键数字必须注明来源表（业务叫法+物理表名）与口径，「没有出处标注的数字等于不可信的数字」；③知识库小节新增「知识条目名纪律」——load 的 name 只能来自大纲/list/search，禁止编造。选表知识本身模型已自主补齐（selections/data_map「按问题找表总地图」+ tables/mixun_camp_product 密训营档案），产品侧只需把交付纪律钉进 preamble。
- **默认报有效口径，别把口径噪音甩给提问的人（第七轮，用户的老板比喻）**：同问题再测，模型知识引用和出处标注都对了，但第一句答「99 单（含 3 单 0 元无效）」——用户比喻：老板问业绩你答 10 个亿，追问才说 9 个亿无效。业务问句默认要的是**有效成交**，第一句就该是剔除无效后的 96 单，技术性总量只放脚注。已钉进交付纪律第 2 条。教训：**口径选择本身就是交付的一部分，数字对但顺序错也是答错**。同轮还实锤「读约定误传给写」：模型把 heading="all" 传给 write_okf_knowledge，落了字面的「# all」垃圾标题——store::write 现在对 heading=all 拒绝并指路（配测试）。
- **「我要的是符合业务理解的数字，不是数据库准确的数字」（第八轮，用户元反馈，全项目最重要的一条）**：定金一问 11 轮对话 182 次调用，暴露根本可用性问题——像「定金」这么简单的概念都要人工给字段级解释模型才能理解，「这样一来可用性就大打折扣」。用户的完整论述：业务方**无从知道**业务概念如何对应表/字段/筛选条件，语义映射（口头表达→业务模型→数据表达）本身就是大模型的职责；说「没有订单表」是因为没找够，公司不可能没有订单表；模型不告诉物理表名用户就无法开权限。已三层落地：① preamble 新增「业务概念：语义映射是你的职责」小节（先给业务理解再给数字/穷尽探查才说没有/权限协作给全清单），并写入用户的公司格言「符合业务理解的数字，不是数据库准确的数字」；② 模型自主沉淀了 playbooks/concept_onboarding（新概念标准接入协议：自描述扫描→多源对账→一次拍板，人工只答一次拍板题）+ selections/deposit_orders_map + concepts/deposit_business_concept；③ users 画像收了逐字原话与「结论/权限清单两段式交付」。教训：**语义层能力是这类产品的生死线，用户讲解概念的每一轮都是产品在漏气；普适观点必须同时进 preamble（立即全域生效）与知识库（工作区细节），只进知识库等于每个新工作区重交一遍学费。**
- **模型把 write 当「整文件保存」用（第十轮，最恶劣的一次知识损坏）**：定金三轮追问后，模型连发三次「load 全文 → 拼自己的合并版 → write_okf_knowledge(heading=某板块名, content=整个文件含 frontmatter)」——文件里嵌出两份 frontmatter 和整段重复内容；更糟的是其中一次 heading=纠错记录 的写入只带了当晚新条目没带全量旧条目，**纠错记录 10 条被 1 条覆盖**。恢复链：git 历史 + jsonl 回执（模型 19:37 的 content 本身就是它做好的合并版，只是丢了个旧条目）+ 18:54 回执里的全量条目，三者拼出最终版。工具侧新守卫：content 以 frontmatter 开头（含 type:/title: 元数据键）直接拒绝——板块写入只收正文（配测试）；纠错记录板块头部定死「追加必须带全量旧条目，先 load 再写」。教训：**「模型把工具当文件保存用」是板块式写入的固有误用模式，守卫要在语义签名层拦（frontmatter 是整文件的特征），而不是指望提示词自觉。**
- **「显示层鬼影」：模型读串查询结果的诬告（2026-09-01 第十一轮，本轮 P0）**：两条 GROUP BY 结果在上下文相邻（dept_lv3 50 行 / dept_lv2 22 行），模型把 lv3 结果里的行读成 lv2 的（推理中「行 #19/#36」超出 22 行范围即读串暴露点），验证探查又全打在 dept_lv2 上——0 行恰恰证明 dept_lv2 干净，却被当成「显示渲染层插鬼影行」的证据，写进知识库并据此**撤回了真实业务发现**（「未匹配到部门」= 无销售跟进的自然转化单，全月 33 笔 100% 转化 52.09 万，复盘直查 Hologres 逐字核实）。payload 与 DB 直查完全一致——查询/渲染链路从未有鬼影。已勘误两处知识（hologres_sql_compat 根因作废、concepts 恢复发现）。教训：**「基础设施有 bug」是最高成本的结论，定案前必须两步全过：payload 对账（summary 声明/meta.rows/rows 三处一致）→ 直查数据库复核；验证探查必须与产生可疑分组的同一列、同一查询同源，跨列 0 行不是证据；行号超出结果行数范围 = 读串了。数仓兜底标签（无/未匹配到部门/未知）大概率是真实 ETL 值，业务上往往正是关键洞察。**
- **错误分类器的词面陷阱（同轮）**：`operator does not exist: text = integer`（text 列 = 数字字面量）与 `function hex(text) does not exist`（Hologres 无 HEX）都因含 "does not exist" 被一锅端进 table_missing，把模型引向排查表/重注册。修复 = `function_missing`/`operator_mismatch` 独立分类 + 对症建议（表缺失建议按是否 `postgres_query` 下推区分修法——下推表禁走短名视图，勿把模型推向拉全表）。**分类器测试用例必须取自真实错误原文**——与「守卫的解析器必须吃 LLM 真实输出形状」同一条纪律的另一面。
- **纠正候选词表第四次校准（同轮）**：补「没有看懂/没看懂/看不懂/结论是什么」——「没有看懂图，所以结论是什么」是结论先行的最高价值反馈，当轮漏报。好消息：知识沉淀后的新会话重测定金两问，开场按大纲直接继承口径与主表、无需重新探索（沉淀→召回→按沉淀交付的验收链路在新会话里走通）。
- **git 提交失败静默吞掉 = 历史断档（2026-08-31 第四轮复盘）**：`run_git_commit` 原来每步 `let _ = cmd.status()`，骨架生成连发几十次 `git add`+`git commit` 撞 `index.lock` 全数静默失败——工作区 okf 里 80+ 表骨架自 08-13 起从未入库，git 审计一上线就现形（⚠️ 未提交变更）。修复：进程内互斥锁串行化提交、失败 `tracing::warn!` 进日志、幂等写（工作区已干净）不算失败；存量 83 文件一次性回填。教训：**「自动提交」不等于「提交成功」，凡 fire-and-forget 的外部进程调用都要么检查状态要么明确说明为什么可以不检查**。
- **候选词表永远追不上模型的语义理解，定位要摆正**：模型按 preamble 语义指引把「说成无归属会让人心生疑惑」正确沉淀进 users，词表却再次漏报——sink 以模型为主，复盘词表只是兜底（补「疑惑」）。别指望关键词枚举穷尽偏好表达。
